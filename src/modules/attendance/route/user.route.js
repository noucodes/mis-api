// src/modules/attendance/routes/user.route.js
  ...
  try {
    ...
    if (employees.length === 0) {
      return res.json({
        users: [],
        source: "No Employees",
        warning: "No employees found. Add at least one record manually first.",
      });
    }

    const nameMap = {};
    employees.forEach((e) => {
      nameMap[String(e.employee_id)] = e.employee_name;
    });
    ...
    for (const emp of employees) {
      ...
      const userLogsToday = (logs.data || [])
        .filter((log) => {
          ...
          return uid === id && t >= targetDate && t < tomorrow;
        })
    ...
    console.log(`Success: Processed ${result.length} employees from ZKTeco device for ${dateStr}`);
    return res.json({ users: result, source: "device" });
  } catch (err) {
    ...
    try {
      ...
      const fallback = await sql`
        SELECT 
          ...
          END AS "statusColor"
        FROM attendance_logs
        WHERE attendance_date = ${dateParam}
        ORDER BY time_in NULLS LAST
      `;
      ...
      return res.status(500).json({
        users: [],
        source: "error",
        error: "System completely offline",
      });
    }
  }
});
